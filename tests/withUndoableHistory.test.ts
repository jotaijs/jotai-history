...
import { withUndoableHistory } from '../src/withUndoableHistory'

describe('withUndoableHistory', () => {
  let store: ReturnType<typeof createStore>
  let baseAtom: PrimitiveAtom<number>
  let historyUndoableAtom: ReturnType<
    typeof withUndoableHistory<PrimitiveAtom<number>>
  >

  beforeEach(() => {
    store = createStore()
    baseAtom = atom(0)
    historyUndoableAtom = withUndoableHistory(baseAtom, 3) // Limit history to 3 entries
